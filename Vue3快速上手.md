...
作用：让路由组件更方便的收到参数（可以将路由参数作为`props`传给组件）,一般组件为在组件标签中传递props
...
//push（）括号内的写法与router-link的to写法一样，字符串或对象
...
3. 示例：
   ...
   ```html
   <!--在父组件中，给子组件绑定自定义事件：-->
   <Child @send-toy="toy = $event 或 回调函数"/>
...
4. 组件标签上的`v-model`的本质：`:moldeValue` ＋ `update:modelValue`事件。
...
6. 如果`value`可以更换，那么就可以在组件标签上多次使用`v-model`
...
## 7.1.【shallowRef 与 shallowReactive 】优化性能
...
## 7.2.【readonly 与 shallowReadonly】保护数据
